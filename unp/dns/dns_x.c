...
>>>>>>>>>>>>>>>>>>>>>>>
	...
	参数: IP地址(网络字节序),地址长度(AF_INET为4), 地址类型.	
...
int main(int argc, char *argv[])
{
	...
	ip_ptr = argv[1];	//char ip_ptr[] = "173.194.127.178"; //www.google.com

	//调用gethostbyname()函数, 返回解析结果(存储于结构体中).
	if((host_ptr = gethostbyaddr(ip_ptr, 4, AF_INET)) == NULL) //调用失败返回NULL
	{
		printf("Error: gethostbyaddr() error for host IP:%s\n", ip_ptr);
		return 0; 
	}
