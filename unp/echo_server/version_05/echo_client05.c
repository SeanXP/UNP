...
void str_cli(FILE *fp, int sockfd)
{
    ...
    {
        FD_ZERO(&fds);
        if(stdin_eof == 0)  //  == 1, stdin eof, 则不需要在关注stdin的可读性;
        ...
        switch(select(maxfdp1, &fds, NULL, NULL, NULL))
        {
            ...
            default:
                ...
                {
                    //从socket中读取返回数据
                    if((num = read(sockfd, buffer, BUFFER_MAX)) <= 0)
                    {
                        if(num == 0 && stdin_eof == 1) // normal termination
                            return ;
                ...
                if(FD_ISSET(fileno(fp), &fds))  // input is readable
                {
                    ...
                    {
                        stdin_eof = 1;
                        shutdown(sockfd, SHUT_WR);      // shutdown() 关闭TCP单向写通道; send FIN to TCP server;
                        FD_CLR(fileno(fp), &fds);
                        continue;
                    }
                }
                break;
